...
#            1) try default.conf for ALTSPATH
...
_tmww_arg_players() {
    if [ -n "${_opt_tmww_use_altdb}" ]; then
        if _cache_invalid TMWW_"${_opt_tmww_servername}" || \
            ! _retrieve_cache TMWW_"${_opt_tmww_servername}"; then

            local update_policy altpath
            zstyle -s ":completion:*:*:tmww:*" cache-policy update_policy
            if [[ -z "$update_policy" ]]; then
                zstyle ":completion:*:*:tmww:*" cache-policy _cache_policy_tmww_players
            fi

            altpath="${_opt_tmww_altspath}/${_opt_tmww_servername}"

            if command -v "${_opt_tmww_jq}" >/dev/null 2>&1; then
                _opt_tmww_players=("${(@f)$(cd ${altpath} && jq -r '.player' dbplayers.jsonl )}")
                _store_cache TMWW_"${_opt_tmww_servername}" _opt_tmww_players
...
_tmww_plugin_alts() {
    local _desc_char="char subcommand completion"
    local _desc_party="party subcommand completion"
    local _desc_player="player subcommand completion"

    local _desc_arg_party="enquoted party name; single quotes recommended"
    local _desc_arg_fuzzy="fuzzy search - case insensitive, l33t chars, one missing and one incorrect character; no spaces; default characters allowed [-/\\.,_a-zA-Z0-9]"
    local _desc_arg_search="case insensitive string"
    local _desc_arg_dumpjson="JSON string, e.g. from player dump operation"
    local _desc_arg_record="record number starting from 1"

    local _desc_arg_rules_player="player name; recommended character set [-_a-z0-9]"
...
(( $+functions[_tmww_plugin_alts_player_list] )) ||
